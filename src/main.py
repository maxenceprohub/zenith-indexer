            ...
            if 0 <= selection < len(fichiers):
                ...
                if contenu.startswith("[ERREUR"):
                    ...
                    if mode_ia.lower() == 'o':
                        ...
                        parametres_chroma = Settings(
                            anonymized_telemetry=False,
                            allow_reset=False,
                            is_persistent=True,
                        )

                        chemin_db = Path(__file__).parent.parent / ".zenith_storage"
                        client_db = chromadb.PersistentClient(path=str(chemin_db), settings=parametres_chroma)

                        collection = client_db.get_or_create_collection(
                            name="zenith_core_index",
                            metadata={"hnsw:space": "cosine"}
                        )

                        existe_deja = collection.get(ids=[f"{fichier_selectionne.name}_chunk_0"])
